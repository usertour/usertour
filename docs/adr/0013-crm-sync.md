...
**A round is alive while its heartbeat is fresh.** `fullSyncStartedAt` is set on claim, refreshed by every page and by every failed attempt that is going to retry, and cleared when the round closes (last page, attempts exhausted, a stalled job, a revoked grant, a disabled integration). A round is presumed dead — taken over by the hourly scan, and no longer shown as running — only after a silence longer than `SYNC_ROUND_STALE_MS`, which is longer than any single wait a page job can take (the ladder's top rung and the `Retry-After` cap, both 12 h; a unit test pins the relation). A superseded round's run is closed as failed; a removed mapping closes its running run.
