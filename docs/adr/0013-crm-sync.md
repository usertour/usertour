...
| Events | `BIZ_EVENT_TRACKED` — not built, see §8 | event in the integration's selected set and the user is linked; ledger message with the event's own topic (`event.tracked.<codeName>`, the same envelope analytics deliveries carry); adapter posts the occurrence with `timestamp = event time` and `id = ledger message id`; with a company mapping the event is also written to the linked company |
