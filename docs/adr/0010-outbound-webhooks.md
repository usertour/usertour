...
- **Reconcile sweep (hourly, `webhookReconcile` queue on the `outbound_cron` prefix)**: with retries living in Redis for a day, a Redis loss must degrade to "delayed up to the sweep cadence", never "stuck PENDING forever". `recordAttempt` now touches the message row on every attempt, making `updatedAt` a last-activity signal; a PENDING message silent for >14h (largest ladder gap + slack) is orphaned. The sweep claims it (CAS on `updatedAt`, the resend discipline) and re-queues a continuation job (`attemptOffset` = logged tries, remaining attempt budget, generation-keyed jobId). An enqueue failure after a claim is deliberately left alone: the claim only bumped `updatedAt`, so the row waits out one more window.
...
- Remaining ledger-consistency edges from the 2026-08-19 review: the listener's single `createMany` batch loses the whole (webhook × event) batch for every endpoint if one webhook row is deleted concurrently (FK violation); the listener re-queries webhooks + entitlement per emit with no cache (candidate for ProjectCacheService when volume warrants).
