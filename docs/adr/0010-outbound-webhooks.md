...
- Failure alerting / auto-disable (delivery log + manual switch cover today's need). Design settled 2026-08-19, two layers: a **time-boxed cooldown** (K consecutive failures → skip-and-record for an exponentially growing window, half-open probe, self-healing — also the answer to one bad endpoint occupying the delivery concurrency slots, in place of per-destination rate limiting) and a **quasi-permanent auto-disable** (sustained failure over a multi-day window → `enabled=false` + notification, human re-enables; the honest-drop ledger rows and post-re-enable Resend already exist). The real cost is the notification channel, not the state machine. Retry backoff should also honor a 429's `Retry-After` instead of the fixed 1/2/4/8s ladder.
