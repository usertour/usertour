...
- **Hand-written `__typename` strings can silently miss the server's actual GraphQL type.** `cache.evict` / `cache.identify` need the exact name Apollo wrote to the cache — which is the server's `@ObjectType` class name, not what the codebase calls the entity. Pluralized model names (`Events`), shared base classes (`BizModel` returned by both Company and base queries), and subclass relationships are easy to get wrong. Cross-check against `apps/server/src/**/models/*.model.ts` when writing or reviewing an `update(cache)` callback; a wrong typename evicts nothing and fails silently.
