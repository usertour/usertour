...
Every user / company / membership write — **including pure literal writes** — runs in a transaction that first takes the entity row with `SELECT … FOR UPDATE`, then computes the merge in TypeScript, then writes. Locking only operation-bearing writes is not enough: an unlocked literal write reads the whole jsonb and writes it back, overwriting a concurrent `add`. The merge is not pushed into a single SQL expression: type validation, the "unchanged → no write, no webhook" short-circuit and the webhook `previousAttributes` diff all live in TypeScript and would have to be duplicated.
