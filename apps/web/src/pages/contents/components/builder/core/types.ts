...
export interface BuilderProviderMethods {
  ...
  fetchContentAndVersion: (contentId: string) => Promise<boolean>;
...
//
// environmentId / projectId are the workspace; contentId is what's being
// edited — the builder always edits the content's editedVersion, so no
// version id is carried. All are fixed props (not async-seeded into the
// draft store): env/project are read tree-wide via
// useEnvironmentId/useProjectId, content is consumed by useBuilderInit to
// hydrate the draft. The Provider therefore knows its complete
// "editing X in workspace Z" identity in one place.
