...

  {
    ...
    fields: [
      ...
      },
      {
        ...
        // Per-type bounds: the full-width bar may be 0 (hidden — a real legacy
        // pattern), the numbered variant is a font size, the rest are strip heights.
        boundsByPath: {
          'progress.height': { min: 0, max: 10 },
          'progress.narrowHeight': { min: 1, max: 10 },
          'progress.chainRoundedHeight': { min: 1, max: 10 },
          'progress.chainSquaredHeight': { min: 1, max: 10 },
          'progress.dotsHeight': { min: 1, max: 10 },
          'progress.numberedHeight': { min: 1, max: 100 },
        },
        suffix: 'px',
        visibleWhen: (s) => s.progress.enabled,
      },
...

  {
    ...
    fields: [
      ...
      },
      {
        ...
        // Bubble offsets shift relative to the chosen position, so negatives are legal.
        offsetMin: -1000,
        offsetMax: 1000,
        labels: {
