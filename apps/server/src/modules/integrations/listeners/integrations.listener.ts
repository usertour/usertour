...
export class IntegrationsListener {
  ...
  private async activeIntegrationsFor(environmentId: string) {
    const integrations = await this.prisma.integration.findMany({
      // Analytics destinations only: a sync-engine row is not an event sink (ADR
      // 0013 §8) and its delivery path expects sync envelopes.
      where: { environmentId, enabled: true, provider: { notIn: [...SYNC_INTEGRATION_PROVIDERS] } },
