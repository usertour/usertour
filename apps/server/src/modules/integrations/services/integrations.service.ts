...
import { buildIntegrationMessage } from '../utils/integration-envelope.util';
import { IntegrationDeliveryJobData, IntegrationEventObject } from '../types/integrations.type';
...
export class IntegrationsService {
   ...
   */
  async upsert(data: IntegrationUpsert, request?: Request) {
   ...
   */
  async updateInbound(data: IntegrationInboundChanges, request?: Request) {
   ...
   */
  async updateSyncEvents(data: IntegrationEventsChanges, request?: Request) {
    const integration = await this.prisma.integration.findUnique({ where: { id: data.id } });
    if (!integration) {
      throw new IntegrationNotFoundError();
    }
