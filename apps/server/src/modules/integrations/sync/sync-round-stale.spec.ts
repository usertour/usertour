 ...
 */
describe('Sync round stale window', () => {
  it('outlasts the longest single wait on the delivery ladder', () => {
    expect(SYNC_ROUND_STALE_MS).toBeGreaterThan(Math.max(...RETRY_DELAYS_MS));
  });

  it('outlasts the Retry-After cap', () => {
    expect(SYNC_ROUND_STALE_MS).toBeGreaterThan(RETRY_AFTER_MAX_MS);
  });
});
