...
import type { CommonResult } from '../types/common-result.type';
...
export class AuthService implements OnModuleInit {
  ...
  async ssoValidate(
    ...
    return await this.prisma.$transaction(async (tx) => {
      ...
      // Carry the invite's publish whitelist onto the membership (as every
      // non-SSO accept path does) — else an editor invite accepted via SSO
      // loses its whitelist. Auto-provision has no invite, so a JIT-created
      // editor starts with none (null = may publish nowhere until an admin
      // extends it).
      await this.teamService.assignUserToProject(
        tx,
        newUser.id,
  ...
  private async ensureSsoProjectAccess(
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      // Carry the invite's publish whitelist onto the membership (see the
      // brand-new-email path); auto-provision (no invite) starts with none.
      await this.teamService.assignUserToProject(
        tx,
        user.id,
        ssoContext.projectId,
  ...
  async signup(payload: NewSignup): Promise<AuthResult> {
    await this.ensureUserRegistrationAllowed();
  ...
  async acceptInvite(payload: InviteAcceptance): Promise<AuthResult> {
  ...
  async resetUserPassword(email: string): Promise<CommonResult> {
  ...
  async resetUserPasswordByCode(code: string, password: string): Promise<CommonResult> {
