...
describe('scope resolvers', () => {
  ...
  describe('Content', () => {
    it('derives project from contentId / versionId / stepId', async () => {
      ...
      expect(await resolvers[ScopeKind.Content]({ stepId: 'step-1' })).toBe('proj-1');
    });
    it('uses explicit environmentId when no entity id is present (create/query)', async () => {
      expect(await resolvers[ScopeKind.Content]({ data: { environmentId: 'env-1' } })).toBe(
        'proj-1',
      );
    });
  ...
  describe('Segment', () => {
    it('derives project from segment id in any of the accepted arg shapes', async () => {
      expect(await resolvers[ScopeKind.Segment]({ data: { id: 'seg-1' } })).toBe('proj-1');
      expect(await resolvers[ScopeKind.Segment]({ data: { segmentId: 'seg-1' } })).toBe('proj-1');
      expect(
  ...
  describe('Integration', () => {
    it('derives project from integrationId → environment', async () => {
      expect(await resolvers[ScopeKind.Integration]({ integrationId: 'int-1' })).toBe('proj-1');
    });
    it('derives project from object-mapping id → integration → environment', async () => {
      expect(await resolvers[ScopeKind.Integration]({ id: 'map-1' })).toBe('proj-1');
    });
