...
- Translate content → list_localizations (target language missing → create_localization), then get_version_localization / update_version_localization per locale (read the guide's \`localization\` section). Without localization scopes, read the locale codes from get_content_version with expand: ["localizations"]. After editing source text, check get_content_version with expand: ["localizations"] for outdated / missing translations BEFORE publishing.
