...
**Targeting "new users" is an audience, not just a frequency.** \`frequency: once\` means "show each matching user a single time" — NOT "only recently-created users"; every user still hits it once. For a true recency audience add a \`user_attribute\` condition on a date attribute (e.g. \`first_seen_at\`, op \`less_than\`, value \`7\` = first seen in the last 7 days), or make a \`segment\` for it and gate on that — then pair it with \`frequency: once\` so new users see it exactly once.
