...
- **tracker**: a \`data.event\` AND \`startRules\` trigger conditions; no theme (it has no UI). A tracker is **headless** — it fires its event whenever its \`startRules\` conditions are met. The event MUST be a **custom** event (its codeName or id from \`list_event_definitions\`; accepted either way, stored as the codeName) — built-in / **system (predefined) events are rejected**: a tracker can only fire custom events, so create one first with \`create_event_definition\` if you don't have a suitable one. Note: a \`current_url\` condition's \`includes\`/\`excludes\` are **arrays** of strings (and \`["*"]\` = the whole site, every page).
