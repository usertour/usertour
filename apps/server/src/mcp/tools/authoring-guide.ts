...
## Themes (required)
Every visual type needs a theme or the SDK renders nothing. Call \`list_themes\` and pass a \`themeId\` to \`create_content\`; if unsure use the one with \`isDefault: true\`. \`create_theme\` / \`update_theme\` start from the default styling and accept a **partial \`settings\`** to brand it (colors, fonts, sizes, …): send only the fields you change — they're field-merged onto the current settings and "Auto" hover/active colors are derived server-side. The \`settings\` arg is a generic object on the tool; call \`get_theme_schema\` for the exact writable fields and their ranges before theming. (Conditional \`variations\` are not yet API-editable — tune those in the theme builder.)
