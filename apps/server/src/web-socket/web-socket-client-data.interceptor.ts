...
export class WebSocketClientDataInterceptor implements NestInterceptor {
  ...
  async intercept(context: ExecutionContext, next: CallHandler): Promise<Observable<any>> {
    ...
    try {
      ...
      socket.data.socketClientData = socketClientData;
      this.logger.debug(`Socket client data fetched and attached for socket ${socket.id}.`);
    } catch (error) {
      this.logger.error(`Failed to fetch socket client data for socket ${socket.id}:`, error);
      socket.data.socketClientData = null;
    }
