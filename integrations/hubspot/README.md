...
- The marketplace listing's **Install button URL** must be the same callback URL, with **partner sign in** enabled: HubSpot then calls the callback with `step=authorize` first (the server sends the customer to the dashboard's install page to sign in and pick an environment) and with `step=finalize` after consent. Without partner sign in, installs from the listing cannot know which environment to connect and fail with an explanation.
